...
import ssl
import certifi
import time # 新增：用於自動播放監聽

# ==========================================
# 0. Android 救命補丁 (放在最上面)
# ==========================================
try:
    # 讓 SSL 憑證生效，解決下載閃退
...
# 【修正 1】強制使用系統鍵盤 (解決輸入法卡住)
Config.set('kivy', 'keyboard_mode', 'system')
Config.set('kivy', 'keyboard_layout', 'system')
# 【修正 2】偽裝成瀏覽器 (解決圖片不顯示)
Config.set('network', 'useragent', 'Mozilla/5.0 (Linux; Android 10) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Mobile Safari/537.36')
...
class MusicEngine(EventDispatcher):
    ...
    def __init__(self, **kwargs):
        ...
        self.player = None
        self.is_monitoring = False # 用來標記是否正在監聽播放進度
    ...
    def start_monitor(self):
        ...
        Clock.schedule_interval(self._check_completion, 1) # 每秒檢查一次
    ...
    def _check_completion(self, dt):
        if self.player:
            try:
                ...
                # 如果播放進度 > 總長度 - 1秒，視為播放結束
                if duration > 0 and current >= (duration - 1000):
                    self.stop_monitor() # 停止監聽
                    self.dispatch('on_track_finished') # 通知 App 換下一首
            except:
                pass
